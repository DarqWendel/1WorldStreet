...
const ProductsPage = () => {
  ...
  const applyFilters = useCallback(() => {
    ...
    if (selectedSizes.length > 0) result = result.filter(p => p.availableSizes && p.availableSizes.some(s => selectedSizes.includes(s)));
  ...
  return (
    <div className="container mx-auto section-padding container-padding">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-extrabold text-primary" style={{ fontFamily: "'Barlow Condensed',sans-serif" }}>
          {searchTerm ? 'Resultados da busca' : 'Nossa Colecao'}
        </h1>
        <p className="text-lg text-muted-foreground mt-2">
          {searchTerm ? filteredProducts.length + ' produto(s) encontrado(s) para "' + searchTerm + '"' : 'Explore o estilo que define as ruas.'}
        </p>
      </motion.div>
      <div className="flex flex-col md:flex-row gap-8">
        <FilterSidebar
          ...
          selectedSizes={selectedSizes} onSizeChange={handleSizeChange}
        />
        <ProductGrid products={filteredProducts} viewMode={viewMode} onViewModeChange={setViewMode} />
      </div>
    </div>
  );
};
